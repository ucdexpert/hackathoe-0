...
## Description
A Claude Agent Task Planner that reads new `.md` files from the `Inbox` folder, analyzes their content, creates step-by-step action plans, and manages file workflow. Integrates with vault-file-manager for moving processed files and maintains comprehensive logging.

## Parameters
- `source_folder` (string, optional): Folder to scan for new files. Default: "Inbox"
- `once` (bool, optional): If True, run once and exit. Default: False (continuous mode)
- `check_interval` (int, optional): Seconds between checks in continuous mode. Default: 30

## Functionality
When invoked, this skill:
1. Scans the Inbox folder for new `.md` files
2. Analyzes file content to understand the task
3. Creates a structured step-by-step plan in `Plan.md` format
4. Places the plan in `Needs_Action` folder
5. Moves processed files from Inbox to Done (via vault-file-manager integration)
6. Logs all actions to `logs/actions.log`
7. Prevents duplicate processing (idempotent via file hash tracking)
8. Runs as a callable skill for watcher or scheduler triggers

## Constraints
- **Idempotent**: Same file is processed only once (tracked by SHA256 hash)
- **No external API calls**: Local vault operations only
- **Silver Tier compliant**: Full audit logging and safe file operations
- **Integration ready**: Callable by vault-watcher or scheduler triggers

## File Tracking
Processed files are tracked in `Logs/task_planner_tracking.json` to prevent duplicate processing.
...
```python
from task_planner import plan_once

result = plan_once()
# Scans Inbox, creates plans, moves to Done
```

### Continuous Monitoring
```python
from task_planner import plan_continuous

result = plan_continuous(interval=30)
# Checks every 30 seconds
```

### Custom Folder
```python
from task_planner import plan_from_folder

result = plan_from_folder("Inbox")
```

### CLI Usage
```bash
# Run once
python scripts/task_planner.py --once

# Continuous mode
python scripts/task_planner.py

# Custom interval
python scripts/task_planner.py -i 20

# Custom folder
python scripts/task_planner.py -f Inbox --once

# Show statistics
python scripts/task_planner.py --stats
```
...
```python
# In vault-watcher's _trigger_ai_workflow():
from task_planner import plan_once
result = plan_once()
```
...
schedule.every(30).seconds.do(plan_once)
...
### Programmatic Use
```python
from task_planner import task_planner_skill
...
├─────────────────────────────────────────────────────────────┤
│  ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌───────┐ │
│  │  Scanner │───▶│ Analyzer │───▶│  Plan    │───▶│ Move  │ │
│  │ (Inbox)  │    │ (Content)│    │ Generator│    │ Done  │ │
│  └──────────┘    └──────────┘    └──────────┘    └───────┘ │
│       │               │               │              │      │
│       ▼               ▼               ▼              ▼      │
│  ┌──────────┐    ┌──────────┐    ┌──────────┐    ┌───────┐ │
│  │  .md     │    │  Task    │    │  Plan    │    │ Done  │ │
│  │  files   │    │  Type    │    │  .md     │    │folder │ │
│  └──────────┘    └──────────┘    └──────────┘    └───────┘ │
│       │                                                       │
│       ▼                                                       │
│  ┌──────────────────────────────────────────────────────────┐│
...
| Type | Keywords | Suggested Actions |
|------|----------|-------------------|
| email | email, send, recipient, subject | Review, approve, send |
| social_media | linkedin, post, social, publish | Review, schedule, publish |
| document_review | report, analysis, document, review | Read, summarize, file |
| scheduling | meeting, schedule, calendar, attend | Check calendar, confirm |
| general | (default) | Review, execute, complete |

## Log Format (actions.log)

```json
{"timestamp": "2026-02-18 14:30:00", "action": "file_detected", "details": {"filename": "task.md"}, "status": "success"}
{"timestamp": "2026-02-18 14:30:01", "action": "content_analyzed", "details": {"task_type": "email", "priority": "normal"}, "status": "success"}
{"timestamp": "2026-02-18 14:30:01", "action": "plan_created", "details": {"plan_path": "Needs_Action/Plan_task_...md"}, "status": "success"}
{"timestamp": "2026-02-18 14:30:02", "action": "file_moved", "details": {"done_path": "Done/task.md"}, "status": "success"}
```

## Tracking Format (task_planner_tracking.json)
...
```json
{
  ...
  "last_metadata": {
    "filename": "task.md",
    "plan_path": "Needs_Action/Plan_task_...md",
    "task_type": "email"
  }
}
```
...
- **Idempotent**: SHA256 hash tracking ensures no file is processed twice
- **Content Analysis**: Automatic task type detection and priority assignment
- **Structured Plans**: Step-by-step action plans with checklists
- **File Management**: Automatic movement to Done folder
- **Dashboard Integration**: Updates Dashboard.md with recent activity
- **Observable**: JSON logging for monitoring and debugging
- **Callable**: Designed for integration with watchers and schedulers
- **Configurable**: Adjustable check interval (10-60 seconds)
